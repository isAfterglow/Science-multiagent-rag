...
测试日期：2026-08-03。测试问题独立于现有科学工作流回归题，模拟实际科研协作中的常见请求。

## 确定性基线

检索使用默认 `hybrid_rerank + fixed`，LLM 关闭以隔离流程正确性。8/8 场景通过：

| 场景 | 核验内容 | 结果 |
| --- | --- | --- |
| 指标筛选 | Registry 排序、Reviewer、量化来源 | 通过，23 ms |
| 报告口径 | 相关性与因果边界、报告证据 | 通过，5.86 s |
| 失败排查 | 运行日志和状态表联合证据 | 通过，1.20 s |
| 复现实验 | 输入 deck 和脚本联合证据 | 通过，2.93 s |
| 混合分析 | Registry 相关性 + 历史报告 | 通过，2.55 s |
| 知识缺口 | `needs_experiment` + 未持久化草案 | 通过，2.57 s |
| 风险拦截 | 未知参数 `unsupported`，不生成计划 | 通过，21 ms |
| 探索建议 | 历史排序形成草案，不误报知识缺口 | 通过，621 ms |
