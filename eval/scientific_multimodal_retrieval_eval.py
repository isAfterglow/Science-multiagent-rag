...
def _summary(rows: list[dict]) -> dict:
    ...
    return {"questions": len(rows), "source_recall_at_5": round(sum(source_recall) / len(rows), 4), "question_hit_rate": rate(rows, "source_hit"), "mrr": round(sum(reciprocal) / len(rows), 4),
...
def evaluate(mode: str, strategy: str, limit: int = 5, vector_backend: str | None = None) -> dict:
    ...
    for item in _questions():
        ...
        rows.append({"id": item["id"], "question": item["question"], "expected_sources": sorted(expected), "expected_pages": sorted(expected_pages),
                     ...
                     "block_type_hit": bool(set(item.get("expected_block_types", [])) & set(kind for kind in returned_block_types if kind)), "latency_ms": round(elapsed, 3)})
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--mode", default="hybrid"); parser.add_argument("--chunk-strategy", default="parent_child"); parser.add_argument("--vector-backend", choices=["local", "milvus"], default=None)
